...
for i in range(len(beat_time)-1):
    (input_music[beat_time[i]*1000:beat_time[i+1]*1000]).export("./test/cool1_"+str(i+1)+".wav", format="wav")# 截取音频的前3秒(单位为毫秒)
...
output_music0.export("./test/cool1_"+str(len(beat_time))+".wav", format="wav") # 保存音频，前面为保存的路径，后面为保存的格式